...
export default function NoteSidebar({
  ...
  return (
    <aside
      ...
      className={cn('flex h-full w-[240px] min-w-[240px] flex-col overflow-y-auto overscroll-contain border-r border-[#e5e2dd] bg-[#f5f3f0]', className)}
    >
      ...
      {pinned.length > 0 ? (
        <div className="mt-5 px-3">
          ...
          {pinned.map((tag) => (
            <TagRow key={`pin-${tag.id}`} tag={{ ...tag, children: [] }} depth={0} activeTagId={activeTagId} onSelect={onSelectTag} onTogglePin={onTogglePinTag} />
          ))}
        </div>
      ) : null}
      ...
      <div className="mt-5 flex-1 px-3 pb-5">
        ...
        {tree.map((tag) => (
          <TagRow key={tag.id} tag={tag} depth={0} activeTagId={activeTagId} onSelect={onSelectTag} onTogglePin={onTogglePinTag} />
        ))}
      </div>
    </aside>
  )
...
function SectionTitle({ children }: { children: string }) {
  return <div className="mb-1.5 px-2 text-[10px] font-semibold uppercase tracking-[0.24em] text-[#8d867f]">{children}</div>
}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active ? 'bg-[#f0eeeb] text-[#3a3733]' : 'text-[#66615b] hover:bg-[#f0eeeb]/70',
      )}
    >
      <Icon size={16} className={active ? 'text-[#3a3733]' : 'text-[#8d867f]'} />
      <span className="flex-1 text-[13px]" style={{ fontWeight: active ? 500 : 400 }}>
        {label}
      </span>
      <span className="text-[11px] text-[#8d867f]">{count}</span>
    </button>
  )
}
    ...
    <div>
      <div
        className={cn(
          ...
          isActive ? 'bg-[#f0eeeb] text-[#3a3733]' : 'hover:bg-[#f0eeeb]/60',
        )}
      ...
      >
        {hasChildren ? (
          <button
            ...
            className="rounded p-0.5 hover:bg-[#3a3733]/5"
          >
        ...
        <Icon size={14} className="text-[#8d867f]" />
        <span className="flex-1 truncate text-[13px]" style={{ fontWeight: isActive ? 500 : 400 }}>
          {tag.name}
        </span>
        <span className="text-[11px] text-[#8d867f] opacity-0 transition-opacity group-hover:opacity-100">{tag.noteCount}</span>
        <button
          type="button"
          onClick={(event) => {
            event.stopPropagation()
          ...
          className="rounded p-0.5 opacity-0 transition-opacity hover:bg-[#3a3733]/5 group-hover:opacity-100"
        >
      ...
      {expanded ? tag.children.map((child) => <TagRow key={child.id} tag={child} depth={depth + 1} activeTagId={activeTagId} onSelect={onSelect} onTogglePin={onTogglePin} />) : null}
    </div>
  )
}
