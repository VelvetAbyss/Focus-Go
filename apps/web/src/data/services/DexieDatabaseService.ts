...
const normalizeTask = (task: TaskItem): TaskItem => {
  ...
  return {
    ...
    ownerId: typeof task.ownerId === 'string' && task.ownerId ? task.ownerId : undefined,
    collaboratorIds: Array.isArray(task.collaboratorIds) ? task.collaboratorIds.filter((item): item is string => typeof item === 'string') : [],
    dependencyTaskIds: Array.isArray(task.dependencyTaskIds) ? task.dependencyTaskIds.filter((item): item is string => typeof item === 'string') : [],
    blockedByTaskIds: Array.isArray(task.blockedByTaskIds) ? task.blockedByTaskIds.filter((item): item is string => typeof item === 'string') : [],
